        ...
        <nav className="backdrop-blur-3xl bg-white/50 p-3">
          <div className="container mx-auto flex justify-between items-center">
            ...
            <div className={`${isMenuOpen ? 'block' : 'hidden'} lg:flex lg:items-center`}>
              ...
              <Link to="/my-rides"
                    className={`block lg:inline-block px-4 py-2 mx-2 rounded-lg ${location.pathname === "/my-rides" && "bg-white/50"}`}>My
                Rides</Link>
              <Link to="/search"
                    className={`block lg:inline-block px-4 py-2 mx-2 rounded-lg ${location.pathname === "/search" && "bg-white/50"}`}>Search
                Rides</Link>
              <Link to="/my-requests"
                    className={`block lg:inline-block px-4 py-2 mx-2 rounded-lg ${location.pathname === "/my-requests" && "bg-white/50"}`}>My
                Requests</Link>
              <FaPowerOff className={"text-red-400"} onClick={() => setModal(true)}/>
            </div>
          </div>
        </nav>
      </div>
